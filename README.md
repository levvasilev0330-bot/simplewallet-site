...
### Шаг 3.1: Настройка кастомного домена (опционально, но рекомендуется)

Для красивого адреса (например, `simplewallet.app`) и автоматического HTTPS:

1. **Приобретите домен** у регистратора (например, Namecheap, GoDaddy, Cloudflare)

2. **Создайте файл CNAME** в корне репозитория:
   ```bash
   echo "yourdomain.com" > CNAME
   ```
   Замените `yourdomain.com` на ваш домен (например, `simplewallet.app`)

3. **Настройте DNS записи** в панели управления доменом:
   
   **Для корневого домена (example.com):**
   - Создайте 4 A-записи, указывающие на IP GitHub:
     - `185.199.108.153`
     - `185.199.109.153`
     - `185.199.110.153`
     - `185.199.111.153`
   ...
   **Для поддомена (www.example.com):**
   - Создайте CNAME-запись:
     - Имя: `www`
     - Значение: `levvasilev0330-bot.github.io`

4. **Настройте в GitHub:**
   - Перейдите: Settings → Pages
   - В поле **Custom domain** введите ваш домен (например, `simplewallet.app`)
   - Поставьте галочку **Enforce HTTPS** (HTTPS будет включён автоматически)
   - Сохраните изменения

5. **Закоммитьте и запушьте CNAME:**
   ```bash
   git add CNAME
...
6. **Ожидайте активации** (может занять до 24 часов):
   - GitHub автоматически создаст SSL-сертификат
   - Проверьте статус в Settings → Pages
   - Когда появится зелёная галочка "Enforce HTTPS" — домен готов!

**Результат:**
- Сайт будет доступен по вашему домену: `https://yourdomain.com`
- HTTPS будет работать автоматически
- Браузер не будет показывать предупреждение о небезопасности
